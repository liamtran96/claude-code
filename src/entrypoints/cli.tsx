...
async function main(): Promise<void> {
  ...
  if (feature('DAEMON') && args[0] === '--daemon-worker') {
    const { runDaemonWorker } = await import('../daemon/workerRegistry.ts');
  ...
  if (feature('DAEMON') && args[0] === 'daemon') {
    ...
    initSinks();
    const { daemonMain } = await import('../daemon/main.ts');
  ...
  if (
    ...
    const { enableConfigs } = await import('../utils/config.js');
    enableConfigs();
    const bg = await import('../cli/bg.ts');
    switch (args[0]) {
      case 'ps':
  ...
  if (
    ...
    const { templatesMain } = await import('../cli/handlers/templateJobs.ts');
  ...
  if (feature('BYOC_ENVIRONMENT_RUNNER') && args[0] === 'environment-runner') {
    ...
    const { environmentRunnerMain } =
      await import('../environment-runner/main.ts');
  ...
  if (feature('SELF_HOSTED_RUNNER') && args[0] === 'self-hosted-runner') {
    ...
    const { selfHostedRunnerMain } =
      await import('../self-hosted-runner/main.ts');
